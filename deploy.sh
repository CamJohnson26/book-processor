...
# Deployment script for book-processor
# Usage: ./deploy.sh <destination_server> <destination_path> [env_file]
# Example: ./deploy.sh user@example.com /home/user/book-processor .env.production
...
if [ $# -lt 2 ]; then
    ...
    echo "Example: $0 user@example.com /home/user/book-processor .env.production"
    exit 1
fi
...
if ! ssh $SSH_OPTS $DESTINATION_SERVER "test -f $DESTINATION_PATH/setup.sh"; then
    echo "Warning: setup.sh not found. Skipping system dependencies installation."
else
...
if ! scp $SSH_OPTS book-processor.service $DESTINATION_SERVER:/tmp/book-processor.service; then
...
if ! ssh $SSH_OPTS $DESTINATION_SERVER "sed -i 's|USER_PLACEHOLDER|$USERNAME|g' /tmp/book-processor.service"; then
    echo "Error: Failed to replace USER_PLACEHOLDER in service file. Exiting."
    exit 1
fi

if ! ssh $SSH_OPTS $DESTINATION_SERVER "sed -i 's|PATH_PLACEHOLDER|$DESTINATION_PATH|g' /tmp/book-processor.service"; then
...
if ! ssh $SSH_OPTS -t $DESTINATION_SERVER "sudo mv /tmp/book-processor.service /etc/systemd/system/"; then
...
if ! ssh $SSH_OPTS -t $DESTINATION_SERVER "sudo systemctl enable book-processor.service"; then
    echo "Error: Failed to enable book-processor service. Exiting."
    exit 1
fi

if ! ssh $SSH_OPTS -t $DESTINATION_SERVER "sudo systemctl stop book-processor.service"; then
    echo "Error: Failed to stop book-processor service. Exiting."
    exit 1
fi

if ! ssh $SSH_OPTS -t $DESTINATION_SERVER "sudo systemctl start book-processor.service"; then
    echo "Error: Failed to start book-processor service. Exiting."
    exit 1
fi

echo "Deployment completed successfully!"
echo "Application is installed as a systemd service and will start automatically on boot"
echo "Service status: ssh $DESTINATION_SERVER 'sudo systemctl status book-processor.service'"
